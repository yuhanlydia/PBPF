...
def test_posterior_update_weight_is_positive_posterior_mean_advantage():
    posterior = np.array([5.0, 1.0, 1.0])
    expected = (5.0 - 1.0) / 7.0
    assert np.isclose(posterior_update_weight(posterior), expected)
    assert np.isclose(
        posterior_update_weight(posterior),
        max(posterior_mean_advantage(posterior), 0.0),
    )


def test_symmetric_or_negative_mean_advantage_does_not_update():
    assert posterior_update_weight([2.5, 7.0, 2.5]) == 0.0
    assert posterior_update_weight([1.0, 1.0, 4.0]) == 0.0
