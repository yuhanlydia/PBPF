...
后续按“运行文档实验、不要求匹配原指标”的补跑记录见 [实验执行清单](EXPERIMENTS_EXECUTED.md)。当前最终代码全套测试 **674 passed**，见 `logs/pytest_after_generation_cap.log`。

代码：`/data/cwj/PBPF`；上游提交：`64b603279740fb716107c37c9911ad80be748801`。
...
- 2 步、1 任务的修复集成检查：完成。
- 1500 步训练、8 任务、6 组对照的完整本地修复诊断：完成。
- 测试：原版 670 passed / 1 failed；确定性修正测试样例后 671 passed。没有跳过失败项。

## 环境

Python 3.12.3；Torch 2.8.0+cu128；CUDA 12.8；Transformers 4.57.6；bitsandbytes 0.48.2；RTX 4090。
...
完整模型缓存入口为 `local/model-cache`，原先位于 SSD `/home/root123/.cache/pbpf/huggingface`，服务器现已将该路径迁移并链接到 `/data/cwj/.cache/pbpf-huggingface-root123`。仓库及实验结果入口位于 `/data/cwj/PBPF`。

## 真实数据预测

官方 RunBugRun v0.0.1 的五个源文件 MD5 和 CodeNet 描述归档 SHA256 均与源码预期一致。当前默认准备参数是 160/32/96 个问题上限、每题最多 4 个候选、每个程序 10 个测试。
过滤后候选数：{'development': 89, 'test': 94, 'train': 353}；问题数：{'development': 32, 'test': 77, 'train': 144}；测试集未来结果数：564。四个测试可见、六个用于未来结果评估。

| 条件 | NLL，均值 ± 样本标准差 | 准确率 |
|---|---:|---:|
| 无证据 | 1.1351 ± 0.0358 | 49.82% |
| 可见结果频率基线 | 0.8230 ± 0.0000 | 82.62% |
| PBPF | 0.5299 ± 0.0068 | 76.24% |
| 打乱关联 | 0.5275 ± 0.0187 | 77.01% |
| 错候选证据 | 0.6857 ± 0.0122 | 71.93% |
| 随机 latent | 1.0950 ± 0.0209 | 46.04% |

三个种子均降低相对无证据和频率基线的 NLL，但打乱关联仍保留收益。full_gate_passes 分别为 false / true / false；不能称为通过完整因果 Gate B。
...
训练样本 120；按 development NLL 选中第 750 步。

| 条件 | 全部测试通过的候选 | 未来测试通过比例 |
|---|---:|---:|
| heuristic | 2/8 | 37.50% |
| no_latent | 1/8 | 25.00% |
| pbpf | 1/8 | 18.75% |
| posterior_map | 1/8 | 18.75% |
| posterior_mean | 1/8 | 25.00% |
| random_latent | 1/8 | 25.00% |

原始生成代码、每个测试的结果和生成耗时保存在 `results/rbr_repair_gate.json`；完整训练记录在 `logs/repair.log`，权重为 `results/rbr_soft_prefix.pt`。
...
代码差异见 `compatibility.patch`。原始失败日志保留在 `logs/pytest.log`，修正后的全套结果在 `logs/pytest_compat.log`；生成上限修复前的完整验收为 671 passed，日志在 `logs/pytest_final.log`。
...
```bash
bash local/verify.sh
bash local/run_smoke.sh
bash local/run_prediction.sh
