...
local/sandbox.sh .venv/bin/python -u local/run_repair_seeded.py \
  ...
  --projector-checkpoint local/results/rbr_soft_prefix.pt \
  --output local/results/rbr_repair_gate.json \
  --train-projector --learning-rate 3e-5 --steps 1500 --tasks 8 --max-sequence-tokens 1024 \
  --maximum-token-rms 0.02 --maximum-delta-rms 0.002 --seed 2701 \
  2>&1 | tee local/logs/repair.log
