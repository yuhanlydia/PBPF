#!/usr/bin/env python3
"""One fixed association-weight intervention after the complete gradient audit."""
import argparse
import contextlib
...
def main():
    ...
    p.add_argument('--gradient-audit', type=Path, required=True)
    p.add_argument('--output', type=Path, required=True)
    args = p.parse_args()
    root = Path(__file__).resolve().parents[1]
    development, audit, output = (p.resolve() for p in (args.development_root, args.gradient_audit, args.output))
    ...
    try:
        ...
        if (prior['status'] != 'complete' or prior['results_sha256'] != file_sha(audit/'results.json')
            ...
            raise ValueError('completed matching gradient audit required')
        evidence = json.loads((audit/'results.json').read_text())
        if (len(evidence['cells']) != 6 or any(
                evidence['cells'][f'new-seed{seed}']['groups']['all']['association_to_base_norm_ratio']['unweighted_batch_mean'] >= .01
                for seed in (1701, 1702, 1703))):
            raise ValueError('complete three-seed weak-association-gradient evidence required')
        ...
        files = [Path(__file__).resolve(), root/'scripts/run_apbpf_prior_training_debug.py',
                 root/'scripts/audit_apbpf_training_gradients.py', root/'scripts/diagnose_apbpf_order_sensitivity.py',
        ...
        plan = {'schema': 'apbpf-association-weight-training-v1', 'seeds': [1701, 1702, 1703],
                ...
                'gradient_audit_sha256': prior['results_sha256'],
                'association_weight': 100., 'reference_association_weight': 1.,
                'model_class': 'run_apbpf_prior_training_debug.PriorTrainingBelief',
                'inference_variant': 'prior_no_resample',
                'hypothesis': 'the small association gradient under prior/no-resampling may require a larger fixed coefficient to influence fitting',
                'design': 'one fixed intervention, association weight1 to100; fresh fits with matched seeds and prior/no-resampling, 1000steps,32particles, unchanged optimizer and other losses; all four strong baselines refit; no adaptive weight sweep',
                ...
                'selection_of_weight': 'two orders of magnitude to move observed fitting-gradient ratio0.0017-0.0031 toward0.17-0.31; fixed before any new fit; no guarantee these local raw-gradient ratios predict Adam updates or generalization',
                'limitations': 'proposal heads remain unused; higher weight can harm likelihood or encourage counterfactual degradation; retain all results and original strong-baseline gates',
        ...
        for seed in plan['seeds']:
            ...
            name = f'association-weight100-seed{seed}'
            state['current'] = name
            ...
            with (output/f'{name}.log').open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
                report = helper.train_and_evaluate(payload, output/f'{name}.json', feature_dim=512,
                    ...
                    association_weight=100., invariance_weight=.1, association_margin=.03,
            ...
            sidecar = {'plan_sha256': state['plan_sha256'], 'model_class': plan['model_class'],
                       'inference_variant': plan['inference_variant'], 'association_weight': 100., 'seed': seed, 'artifact_receipt': receipt}
