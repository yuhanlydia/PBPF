#!/usr/bin/env python3
"""DeepSeek RBR replication with eight independently seeded serial decodes."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    p.add_argument('--model-proof', type=Path, required=True)
    p.add_argument('--family', choices=['deepseek'], default='deepseek')
    ...
    weights = [item for item in proof['files'] if item['file'].endswith('.safetensors')]
    if (proof['model'] != model_id or proof['revision'] != revision or not weights
            or any(item['sha256'] != item['publisher_lfs_sha256'] for item in weights)):
        raise ValueError('pinned model proof requires nonempty publisher-verified weights')
    root = Path(__file__).resolve().parents[1]
    config = {'schema': 'apbpf-rbr-generation-v1', 'family': args.family, 'model': model_id, 'revision': revision,
              'source_sha256': sha(__file__), 'model_proof_sha256': sha(args.model_proof),
              ...
              'temperature': .8, 'top_p': .95, 'decode_batch_size': 1,
    ...
    for i, row in enumerate(rows):
        ...
        for j in range(8):
            candidate_seed = int.from_bytes(hashlib.sha256(
                f'{args.seed}:{row["task_id"]}:{j}'.encode()).digest()[:4], 'big')
            ...
            candidates.append({'candidate_id': f'{row["task_id"]}/{args.family}/{j}', 'code': extract_program(text),
                               'raw_completion': text, 'decode_seed': candidate_seed, 'generated_tokens': count, 'hit_token_cap': tokenizer.eos_token_id not in ids})
