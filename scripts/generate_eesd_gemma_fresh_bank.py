#!/usr/bin/env python3
"""Generate one adapter-backed Gemma 3 4B candidate per public primary source."""
import argparse
import hashlib
...
def tree_sha(directory):
    ...
    if not files:
        raise ValueError('fresh policy requires a nonempty LoRA adapter')
...
def source_rows(domain):
    if domain == 'runbugrun':
        ...
        public = ROOT / 'runs/eesd-data/replay-extension-locked-20260920' / domain / 'public'
    ...
    if expected != sha(tasks_path):
        raise ValueError('public primary inventory checksum mismatch')
    if domain.endswith('_replay') and manifest.get('schema') != 'eesd-replay-public-v1':
        raise ValueError('Replay primary manifest differs')
    rows = [json.loads(line) for line in tasks_path.read_text().splitlines()
            if json.loads(line)['split'] == 'primary']
    if domain == 'codearc':
        grouped = {}
        for row in rows:
    ...
    if len(rows) != expected_count or len({row['source_component_id'] for row in rows}) != expected_count:
        raise ValueError('unexpected Gemma primary source population')
    return public, rows
...
def main():
    ...
    if args.offset < 0 or args.components < 1 or args.offset + args.components > len(all_rows):
        p.error('invalid primary shard bounds')
    ...
    if args.preflight_only:
        print(json.dumps({'status': 'Gemma-fresh-preflight', 'domain': args.domain,
    ...
    run = {'schema': f'apbpf-{"rbr" if args.domain == "runbugrun" else "codearc"}-generation-v1'
           if args.domain in ('runbugrun','codearc') else 'eesd-gemma-replay-fresh-generation-v1',
           'domain': args.domain, 'split': 'primary', 'seed': args.seed,
           ...
           'decode_policy': 'temperature0.8-top_p0.95',
    ...
    if run_file.exists():
        if json.loads(run_file.read_text()) != run:
            raise ValueError('Gemma fresh bank resume identity changed')
    elif any(args.output.iterdir()):
        raise ValueError('orphan Gemma fresh bank artifacts')
    else:
        publish(run_file, run)
    if list(args.output.glob('*.partial')):
        raise ValueError('partial Gemma fresh record requires audit')
    ...
    for row, rendered, ids, metadata in prepared:
        ...
        if path.is_file():
            if not path.with_suffix('.sha256').is_file() or path.with_suffix('.sha256').read_text().strip() != sha(path):
                raise ValueError('Gemma fresh record checksum differs')
        else:
    ...
    if pending:
        ...
        for index, (row, rendered, ids, metadata, path) in enumerate(pending, 1):
            ...
            if inputs.input_ids.shape[1] != len(ids):
                raise ValueError('Gemma fresh prompt token identity differs')
            ...
            record = {'task_id': row['task_id'], 'source_component_id': row['source_component_id'],
                      'split': 'primary', 'seed': seed, 'prompt_sha256': metadata['rendered_prompt_sha256'],
    ...
    publish(args.output / 'complete.json', {'schema': 'eesd-gemma-fresh-bank-complete-v1',
                                           'run_sha256': sha(run_file), 'files': files})
    load_bank(args.output)
    print(json.dumps({'status': 'Gemma-fresh-bank-complete', 'components': len(files)}), flush=True)
