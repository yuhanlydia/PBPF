...
def main():
    ...
    proof = json.loads(args.model_proof.read_text())
    if proof["model"] != MODEL or proof["revision"] != REVISION or not all(
            row["sha256"] == row["publisher_lfs_sha256"] for row in proof["files"] if row["file"].endswith(".safetensors")):
        raise ValueError("replication model proof mismatch")
    ...
    config = {"schema": "apbpf-codearc-generation-v1", "model": MODEL, "revision": REVISION,
              "source_sha256": sha(__file__), "model_proof_sha256": sha(args.model_proof), "prompt_source_sha256": sha(Path(__file__).resolve().parents[1] / "src/pbpf/apbpf/codearc_prompt.py"),
              "prompt_policy": "public fields clipped to 2048 characters each; token cap 4096; all groups retained", "public_manifest_sha256": sha(args.public_root / "manifest.json"),
    ...
    for index, row in enumerate(tasks):
        ...
        prompt = tokenizer.apply_chat_template([
            ...
            {"role": "user", "content": bounded_task_text(row)}], tokenize=False, add_generation_prompt=True)
        ...
        for candidate_index, tokens in enumerate(continuations):
            ...
            candidates.append({"candidate_id": f"{row['task_id']}/deepseek/{candidate_index}",
        ...
        result = {"task_id": row["task_id"], "source_component_id": row["source_component_id"],
                  ...
                  "public_prompt_bounded": bounded_task_text(row) != row["task_text"],
                  "input_tokens": int(inputs.input_ids.shape[1]), "candidates": candidates, "elapsed_seconds": time.monotonic() - begin}
